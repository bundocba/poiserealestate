...
class JFormFieldCondition extends JFormFieldList
{
    ...
    public function getOptions($available = false)
	{
        ...
        $query->select('DISTINCT(c.id), c.id AS value, c.title AS text')
            ->from('#__iproperty_condition as c');
        if($available){
            $query->join('INNER','#__iproperty AS p ON p.condition = c.id');
        }
        ...
        {            
            ...
            array_unshift($options, JHtml::_('select.option', '', JText::_('COM_IPROPERTY_CONDITION')));
        }
