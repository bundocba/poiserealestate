...
class IpropertyModelPropertyagent extends JModelAdmin
{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_iproperty.propertyagent', 'propertyagent', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_iproperty.edit.propertyagent.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('propertyagent.id') == 0) {
                ...
                $data->tags->getTagIds($this->getState('propertyagent.id'), 'com_iproperty.propertyagent');
    ...
    protected function prepareTable($table)
	{
		...
		foreach ($table as $key => $value){
			if (isset($value))	$table->$key = trim($value);
		}
