...
class IpropertyViewPropertyTypes extends JViewLegacy
{
    protected $items;
    ...
    public function display($tpl = null)
	{
        ...
        // Load the submenu.
		IpropertyHelper::addSubmenu(JFactory::getApplication()->input->getCmd('view', 'propertytypes'));
	...
	{
		JToolBarHelper::title(JText::_('COM_IPROPERTY_PROPERTYTYPES'), 'iproperty.png');

        // Only show these options to super agents or admin
        if ($this->ipauth->getAdmin()){
            JToolBarHelper::addNew('propertytype.add', 'JTOOLBAR_NEW');
        }
        if ($this->ipauth->getAdmin() || $this->ipauth->getSuper()){
            JToolBarHelper::editList('propertytype.edit', 'JTOOLBAR_EDIT');
        }
        if ($this->ipauth->getAdmin()){   
            JToolBarHelper::divider();
            IpToolbar::featureList('propertytypes.feature', JText::_('COM_IPROPERTY_FEATURE'));
            IpToolbar::unfeatureList('propertytypes.unfeature', JText::_('COM_IPROPERTY_UNFEATURE'));
            JToolBarHelper::divider();
            JToolBarHelper::publishList('propertytypes.publish', 'JTOOLBAR_PUBLISH');
            JToolBarHelper::unpublishList('propertytypes.unpublish', 'JTOOLBAR_UNPUBLISH');
            JToolBarHelper::divider();
            JToolBarHelper::deleteList(JText::_('COM_IPROPERTY_CONFIRM_DELETE'), 'propertytypes.delete', 'JTOOLBAR_DELETE');
		}
        
        // Add search filters
		JHtmlSidebar::setAction('index.php?option=com_iproperty&view=propertytypes');
