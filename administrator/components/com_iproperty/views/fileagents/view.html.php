...
class IpropertyViewFileAgents extends JViewLegacy
{
    protected $items;
    ...
    public function display($tpl = null)
    {
        ...
        // Load the submenu.
		IpropertyHelper::addSubmenu(JFactory::getApplication()->input->getCmd('view', 'files'));
	...
	{
		JToolBarHelper::title(JText::_('COM_IPROPERTY_FILEAGENTS'), 'iproperty.png');

        // Only show these options to super agents or admin
        if ($this->ipauth->getAdmin()){
            JToolBarHelper::addNew('fileagent.add', 'JTOOLBAR_NEW');
        }
        if ($this->ipauth->getAdmin() || $this->ipauth->getSuper()){
            JToolBarHelper::editList('fileagent.edit', 'JTOOLBAR_EDIT');
        }
        if ($this->ipauth->getAdmin()){   
            JToolBarHelper::divider();
            JToolBarHelper::divider();
            JToolBarHelper::publishList('fileagents.publish', 'JTOOLBAR_PUBLISH');
            JToolBarHelper::unpublishList('fileagents.unpublish', 'JTOOLBAR_UNPUBLISH');
            JToolBarHelper::divider();
            JToolBarHelper::deleteList(JText::_('COM_IPROPERTY_CONFIRM_DELETE'), 'fileagents.delete', 'JTOOLBAR_DELETE');
		}
        
        // Add search filters
		JHtmlSidebar::setAction('index.php?option=com_iproperty&view=fileagents');
