...
class IpropertyViewTraining extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
    ...
    protected function addToolbar()
	{
		...
		JToolBarHelper::title($isNew ? JText::_('COM_IPROPERTY_ADD_TRAINING') : JText::_('COM_IPROPERTY_EDIT_TRAINING').': '.$this->item->name, 'iproperty.png');
        ...
        if ($this->ipauth->getAdmin()){
            JToolBarHelper::custom('training.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);

            // If an existing item, can save to a copy.
            if (!$isNew) {
                JToolBarHelper::custom('training.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
            }
            JToolBarHelper::divider();
        }        
		...
		if (empty($this->item->id))  {
			...
			JToolBarHelper::cancel('training.cancel', 'JTOOLBAR_CLOSE');
		}
	}
