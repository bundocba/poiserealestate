...
class IpropertyViewTenures extends JViewLegacy
{
    protected $items;
    ...
    public function display($tpl = null)
	{
        ...
        // Load the submenu.
		IpropertyHelper::addSubmenu(JFactory::getApplication()->input->getCmd('view', 'tenures'));
	...
	{
		JToolBarHelper::title(JText::_('COM_IPROPERTY_TENURES'), 'iproperty.png');

        // Only show these options to super agents or admin
        if ($this->ipauth->getAdmin()){
            JToolBarHelper::addNew('tenure.add', 'JTOOLBAR_NEW');
        }
        if ($this->ipauth->getAdmin() || $this->ipauth->getSuper()){
            JToolBarHelper::editList('tenure.edit', 'JTOOLBAR_EDIT');
        }
        if ($this->ipauth->getAdmin()){   
            JToolBarHelper::divider();
            IpToolbar::featureList('tenures.feature', JText::_('COM_IPROPERTY_FEATURE'));
            IpToolbar::unfeatureList('tenures.unfeature', JText::_('COM_IPROPERTY_UNFEATURE'));
            JToolBarHelper::divider();
            JToolBarHelper::publishList('tenures.publish', 'JTOOLBAR_PUBLISH');
            JToolBarHelper::unpublishList('tenures.unpublish', 'JTOOLBAR_UNPUBLISH');
            JToolBarHelper::divider();
            JToolBarHelper::deleteList(JText::_('COM_IPROPERTY_CONFIRM_DELETE'), 'tenures.delete', 'JTOOLBAR_DELETE');
		}
        
        // Add search filters
		JHtmlSidebar::setAction('index.php?option=com_iproperty&view=tenures');
