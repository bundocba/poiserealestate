...
class com_ipropertyInstallerScript
{
    ...
    function preflight($action, $parent)
    {
        ...
        if( version_compare( $jversion->getShortVersion(), $this->minimum_joomla_release, 'lt' ) ) {
            Jerror::raise(E_WARNING, null, 'Cannot install Intellectual Property '.$this->release.' in a Joomla release prior to '.$this->minimum_joomla_release);
            return false;
        }
        ...
        switch ($action){
            case 'update':
                ...
                if ( version_compare( $this->release, $oldRelease, 'lt' ) ) {
                    Jerror::raise(E_WARNING, null, 'Incorrect version sequence. Cannot upgrade Intellectual Property ' . $rel);
                    return false;
                }
    ...
    function postflight($action, $parent)
    {
        ...
        // Fix for IP3.1.2 bug by not adding developer to iproperty table in install sql, but only when
        // updating. Will break backup/restore if this column does't exist. 
        $db     = JFactory::getDbo();
        $fields = $db->getTableColumns('#__iproperty');
        if(!array_key_exists('developer', $fields))
        {
            $query = 'ALTER TABLE '.$db->quoteName('#__iproperty').' ADD '.$db->quoteName('developer').' VARCHAR(100) NOT NULL AFTER '.$db->quoteName('property_name');
            $db->setQuery($query);
            $db->execute();
        }
        // end fix               
