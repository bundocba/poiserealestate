...
class UsersControllerUser extends UsersController
{
	...
	public function login()
	{
		...
		if($_POST['agent']){
			$alicense  = $input->$method->get('username', '', 'USERNAME');
			$db = JFactory::getDBO();
			$query = "SELECT user_id FROM #__iproperty_agents WHERE alicense='".$alicense."'";
			$db->setQuery($query);
			$user_id = $db->loadResult();
			
			//get username 
			if(!empty($user_id)){
				$query2 = "SELECT username FROM #__users WHERE id=".$user_id;
				$db->setQuery($query2);
			...
			if (true === $app->login($credentials, $options))
			{
				...
				//print_r($agent_id);die;
				
				
				if($agent_id){
					$app->redirect(JRoute::_('index.php?option=com_iproperty&view=file&Itemid=224&id='.$agent_id, false));
				}else{
			...
			if (true === $app->login($credentials, $options))
			{
				...
				if($_POST['agent']){
					...
					$db = JFactory::getDBO();
					$query = "SELECT id FROM #__iproperty_agents WHERE uid=".$user->id;
					$db->setQuery($query);
					$agent_id = $db->loadResult();
